...
public class VideoPlayerController : MonoBehaviour
{
    ...
    // ───────────────────────────────────────────
    // PLAYERPREFS KEYS (persist volume & mute antar scene / restart)
    // ───────────────────────────────────────────
    private const string PREF_VOLUME_KEY = "VideoPlayer_Volume";
    private const string PREF_MUTED_KEY = "VideoPlayer_Muted";
    ...
    void Start()
    {
        ...
        // Volume slider - load state tersimpan dari PlayerPrefs (fallback: volume 1, tidak mute)
        float savedVolume = PlayerPrefs.GetFloat(PREF_VOLUME_KEY, 1f);
        isMuted = PlayerPrefs.GetInt(PREF_MUTED_KEY, 0) == 1;
        lastVolume = savedVolume > 0f ? savedVolume : 1f;

        // Set tampilan slider & icon sesuai state tersimpan (tanpa trigger listener)
        volumeSlider.SetValueWithoutNotify(isMuted ? 0f : savedVolume);
        volumeSlider.interactable = !isMuted;
        volumeSlider.onValueChanged.AddListener(OnVolumeChanged);

        if (muteIcon) muteIcon.sprite = isMuted ? muteSprite : soundSprite;
    ...
    void OnFirstFrameReady(VideoPlayer vp, long frameIdx)
    {
        ...
        // Terapkan juga volume audio-nya (dulu nggak pernah di-set eksplisit di awal)
        vp.SetDirectAudioVolume(0, isMuted ? 0f : lastVolume);
    ...
    void OnMuteClicked()
    {
        ...
        if (isMuted)
        {
            lastVolume = volumeSlider.value > 0f ? volumeSlider.value : lastVolume; // simpan posisi sebelumnya
        ...
        // Simpan state mute ke PlayerPrefs
        PlayerPrefs.SetInt(PREF_MUTED_KEY, isMuted ? 1 : 0);
        PlayerPrefs.Save();
    }
    ...
    void OnVolumeChanged(float value)
    {
        ...
        // Track & simpan volume terakhir yang di-set user
        if (value > 0f) lastVolume = value;

        PlayerPrefs.SetFloat(PREF_VOLUME_KEY, value);
        PlayerPrefs.Save();
    }
