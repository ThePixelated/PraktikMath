...
public class SoalManager : MonoBehaviour
{
    ...
    private void Update()
    {
        if (!isSoalEnd && !_isPaused)
        {
            ...
            else
            {
                ...
                // Isi result data
                SoalResultData.TotalSoal = _randomizeSO_SoalData.Count;
                SoalResultData.WrongCount = _wrongCount;
    ...
    {
        if (isSoalEnd) return; // kuis sudah selesai, abaikan klik beruntun
        ...
        if (value != _currentSoal.JawabanSoal)
        {
            ...
            if (_wrongCount - 1 < wrongIndicatorImage.Count)
                wrongIndicatorImage[_wrongCount - 1].sprite = wrongIndicator;

            // Gagal karena salah 3x -> langsung akhiri, jangan proses soal berikutnya.
            if (_wrongCount >= 3)
            {
                isSoalEnd = true;
                SoalResultData.TotalSoal = _randomizeSO_SoalData.Count;
